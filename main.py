...
def summarize_roberta(results_dir: Path) -> None:
    ...
    if training_summary:
        ...
        print(
            "  mixed_train_examples={mixed} | real_train={real} | selected_synthetic={synthetic} | synthetic_share={share}".format(
                mixed=training_summary.get("mixed_train_examples", "n/a"),
                real=training_summary.get("real_train_examples", "n/a"),
                synthetic=training_summary.get("selected_synthetic_examples", "n/a"),
...
def summarize_error_and_diagnostics(results_dir: Path) -> None:
    ...
    mixed_predictions = metrics_from_prediction_csv(results_dir / "roberta_mixed_post_ocr/test_predictions.csv")
    prediction_rows = read_csv_rows(results_dir / "roberta_mixed_post_ocr/test_predictions.csv")
    if mixed_predictions and prediction_rows:
        false_negatives = 0
        false_positives = 0
        correct = 0
        for row in prediction_rows:
            ...
            if true_label == predicted_label:
                ...
                false_positives += 1
        print("RoBERTa mixed post/OCR held-out error breakdown:")
        print(
            f"  correct={correct} | false_negatives={false_negatives} | false_positives={false_positives} | "
            f"macro_f1={format_float(mixed_predictions.get('macro_f1'))}"
        )

    diagnostics = read_json(results_dir / "roberta_mixed_synthetic/synthetic_diagnostic_metrics_threshold_0p5.json")
    if diagnostics:
        print("\nSynthetic diagnostic behavior for RoBERTa mixed real + synthetic:")
        rows = [
            ["Explicit recall", format_float(diagnostics.get("explicit_recall"))],
            ["Implicit recall", format_float(diagnostics.get("implicit_recall"))],
            ["Neutral false-positive rate", format_float(diagnostics.get("neutral_false_positive_rate"))],
            ["Emoji pair flip rate", format_float(diagnostics.get("emoji_flip_rate"))],
            ["Emoji mean delta p", format_float(diagnostics.get("emoji_mean_delta_p"))],
        ]
        print_table(["Metric", "Value"], rows)

    optional_summary = read_csv_rows(results_dir / "analysis_step4_roberta_mixed_post_ocr/edos_implicit_explicit_summary.csv")
    if optional_summary:
        print("\nEDOS explicit/implicit analysis:")
        rows = [
            [
        ...
        print_table(["Type", "Samples", "Recall", "Macro F1"], rows)
